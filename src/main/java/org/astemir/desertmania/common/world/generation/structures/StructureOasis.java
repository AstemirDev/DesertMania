...
public class StructureOasis extends Structure{
    ...
    public static final SimplifiedSchematic SCHEMATIC = new SimplifiedSchematic("oasis.schem").
            replacement((state) -> {
    ...
    public static final Codec<StructureOasis> CODEC = simpleCodec(StructureOasis::new);

    protected StructureOasis(StructureSettings settings) {
        super(settings);
    }
        ...
        if (y < context.chunkGenerator().getSeaLevel()-10){
            return onTopOfChunkCenter(context, Heightmap.Types.WORLD_SURFACE_WG, (builder) -> {
                ...
                for (int i = 0; i < PLACEMENTS.length; i++) {
                    ...
                    builder.addPiece(new StructureOasis.Piece(i,finalPieceX,y,finalPieceZ,(int)rectangle.getWidth(),(int)rectangle.getHeight()));
                }
            });
        }else{
    ...
    public static class Piece extends ScatteredFeaturePiece {
        ...
        public Piece(int index,int pX, int pY, int pZ,int width,int depth) {
            super(DMStructures.PieceTypes.OASIS_PIECE.get(), pX, pY, pZ, width,SCHEMATIC.getHeight(),depth, Direction.NORTH);
            this.index = index;
        }

        public Piece(CompoundTag pTag) {
            super(DMStructures.PieceTypes.OASIS_PIECE.get(), pTag);
    ...
    public StructureType<?> type() {
        return DMStructures.Types.OASIS.get();
    }
}

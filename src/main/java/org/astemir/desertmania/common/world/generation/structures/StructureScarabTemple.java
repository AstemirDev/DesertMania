...
public class StructureScarabTemple extends Structure{

    public static final SimplifiedSchematic SCHEMATIC = new SimplifiedSchematic("scarab_temple.schem").
            replacement((state) -> {
    ...
    public static final Codec<StructureScarabTemple> CODEC = simpleCodec(StructureScarabTemple::new);

    protected StructureScarabTemple(StructureSettings settings) {
        super(settings);
    }
        ...
        if (y < context.chunkGenerator().getSeaLevel()-10){
            return onTopOfChunkCenter(context, Heightmap.Types.WORLD_SURFACE_WG, (builder) -> {
                ...
                for (int i = 0; i < PLACEMENTS.length; i++) {
                    ...
                    builder.addPiece(new StructureScarabTemple.Piece(i,finalPieceX,y,finalPieceZ,(int)rectangle.getWidth(),(int)rectangle.getHeight()));
                }
            });
        }else{
    ...
    public static class Piece extends ScatteredFeaturePiece {
        ...
        public Piece(int index,int pX, int pY, int pZ,int width,int depth) {
            super(DMStructures.PieceTypes.SCARAB_TEMPLE_PIECE.get(), pX, pY, pZ, width,SCHEMATIC.getHeight(),depth, Direction.NORTH);
            this.index = index;
        }

        public Piece(CompoundTag pTag) {
            super(DMStructures.PieceTypes.SCARAB_TEMPLE_PIECE.get(), pTag);
    ...
    public StructureType<?> type() {
        return DMStructures.Types.SCARAB_TEMPLE.get();
    }
}
